...
# Floorplan
################################################################
set sram_height 110.061
set sram_width 234.268
set num_sram 64

set ring_offset 1.672
set macro_offset [expr $ring_offset * 15]
set core_offset [expr $ring_offset * 10]

set y_spacing [expr $ring_offset * 81]
set x_spacing [expr $ring_offset * 156]

set y_down_min [expr $core_offset + $sram_height]
set y_up_min [expr $y_down_min + $y_spacing * 6]
set y_side_min [expr $y_down_min + $y_spacing * 4]

set x_min [expr $core_offset]
set x_right_min [expr $x_min + $x_spacing * 4]

initialize_floorplan -shape R -boundary {{0 0} {1834.184 1362.68}} -core_offset {50.0 50.0}

#soft_blockage
set COREBND [get_attribute [get_core_area] bbox]
set MACROX [expr $sram_width * 3 + ($core_offset + $macro_offset) * 2]
set MACROY [expr $sram_height * 4 + $core_offset * 2 + $macro_offset * 3]
...
foreach x $MACROBBOX {
		  set llx [expr [lindex $x 0 0] + 1] 
		  set urx [expr [lindex $x 1 0] - 1]
		  set lly [expr [lindex $x 0 1] + 1]
		  set ury [expr [lindex $x 1 1] - 1]
...
-pattern { {pattern: Macro_ring} {nets: {VSS VDD}}  {offset: {1.672 1.672}} } ;#set strategy for Macros / -pattern {pattern name} {net name} {offset}
...
set M8_MESH_bdry  "{0 -36.564} {1834.1840 1399.2440}"
...
set_app_options -name opt.dft.clock_aware_scan_reorder -value false ;#This app option check hold violation status in scan chain first, and if there exists hold violations along scan path, clock driver aware reordering only will be employed to reduce the hold violations along scan path witin each scan chain to save the hold fixing resource on scan path
...
write_sdc -output ./outputs/${TOPDESIGN}.sdc
