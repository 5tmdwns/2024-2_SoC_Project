...
# Floorplan
################################################################
source ./place_macro_BISR.tcl
set core_x [expr 8.732 + [lindex [get_attribute [get_cells "USRAM/genblk1[63].UMEM"] bbox] 1 0]]
set core_y [expr 18.392 + [lindex [get_attribute [get_cells "SPARE_USRAM/genblk1[23].SPARE_UMEM"] bbox] 1 1]]
...
set MACROX [expr 2*($sram_x_size + $gap_x )]; #FSM 공간
set MACROY [expr 4*($sram_y_size + $gap_y )]	;	
...
set STDBND_11 [expr [lindex [lindex $COREBND 1] 1] - [expr $MACROY + 4 * ($BISR_y_size + $BISR_gap_y)]]
set STDBND "{$STDBND_00 $STDBND_01} {$STDBND_10 $STDBND_11}"

set MACROX2 [expr 3*($sram_x_size + $gap_x )]; #FSM 공간
#set MACROY2 [expr 4*($sram_y_size + $gap_y )]	;	
set STDBND2_00 [expr [lindex [lindex $COREBND 0] 0] + $MACROX2] ; # {0,0}에서 0번째 원소 여기선MACROY가 커질수록 boundary가 작아짐
set STDBND2_01 [expr [lindex [lindex $COREBND 0] 1] + [expr $BISR_y_size + $BISR_gap_y]] ;
set STDBND2_10 [expr [lindex [lindex $COREBND 1] 0] - $MACROX2] ;
set STDBND2_11 [expr [lindex [lindex $COREBND 1] 1] - [expr $BISR_y_size + $BISR_gap_y]] ; # {2000,2000}에서 1번째 원소
